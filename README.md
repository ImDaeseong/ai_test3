...
- 스택: Next.js(App Router) + TypeScript + Tailwind CSS, Zod(요청/응답 검증), Vitest/Testing Library(단위·컴포넌트 테스트)/Playwright(E2E). LLM/RAG 기반 분석, provider 연동 전 mock-first UI 원칙. 별도 서브프로젝트 `jobkorea-ai/`(Python/FastAPI, `app/`과 무관한 독립 채용정보 수집·분류 서브프로젝트, 2026-07-24 `pipeline/`에서 전환).
