...
Score generation is implemented as a separate feature under `app/notation/` (`docs/features/score_generation.md` has the design and implementation log). The MusicXML lead-sheet/session-chart export is done, and melody note events now come from an optional `ScoreTranscriber` provider chain (`basic-pitch` -> local autocorrelation heuristic -> section-energy chart fallback) instead of section energy alone. Confidence and limitations are labeled directly in every export ("transcription guide", not publication-ready notation). Chord and beat/downbeat extraction are still not implemented (`ROADMAP.md` Phase 7).
