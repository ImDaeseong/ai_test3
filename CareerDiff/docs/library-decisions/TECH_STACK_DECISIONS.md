...
- `../../prompts/MANUAL_ANALYSIS_PROMPT.md` — a copy-paste version of `buildAnalysisPrompt.ts` + `analysisResult.ts`'s zod schema, kept in sync by hand with the code.
- `../../prompts/WEB_PROJECT_USAGE.md` — Claude/ChatGPT Project setup steps and a manual JSON-output verification checklist (schema-shape, exactly 3 mini projects, Korean output, no fabricated evidence, `retrievalContext`/`metadata` flags correct).
