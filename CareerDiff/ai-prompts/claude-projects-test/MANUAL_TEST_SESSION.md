...
`docs/design/AI_EVALUATION_PLAN.md`가 정의한 10개 평가 유형을 미리 합쳐놓은 실행지다.
플레이스홀더를 채울 필요 없이 각 세트의 "복사할 프롬프트" 블록을 그대로 복사해서 Claude Project에
붙여넣으면 된다.

세트-유형 대응표 (`AI_EVALUATION_PLAN.md`의 10개 유형 기준):
...
- [ ] `jobRequirements.requiredSkills`에 Java, Spring Boot, MySQL, REST API 설계가 필수로 명시
- [ ] `candidateEvidence`가 "여러 프로젝트", "다양한 기능", "데이터베이스 관련 작업" 같은 모호한 원문을 sourceSnippet에 담되, `confidence`를 낮게(low/medium) 부여 — 구체 증거 없이 high로 부풀리지 않음
- [ ] `matches.strong`에 Spring Boot/MySQL 관련 항목이 없음(구체 증거 부재) — Java 언급만으로 strong 처리하지 않음
- [ ] `matches.weak`에 "백엔드 개발 경험은 있으나 Spring/MySQL 등 구체 기술 증거 부족" 취지의 항목 포함
- [ ] `resumeSuggestions`가 모호한 표현을 구체화하라고 제안하되, 없는 사실(Spring Boot 3년 등)을 지어내지 않음
- [ ] `fitScore.total`이 중간 이하이고, reason이 "증거가 모호함/구체성 부족"을 명시적으로 언급
...
- [ ] `matches.strong`에 React/JavaScript/HTML-CSS/Git이 정직하게 강한 매치로 포함(실제로 요건을 크게 초과 충족)
- [ ] `matches.risks` 또는 `summary`에 "포지션(주니어) 대비 후보자 시니어리티가 과도하게 높다"는 취지의 과잉자격 리스크가 명시적으로 언급됨
- [ ] `fitScore.total`이 스킬 매치만으로 기계적으로 100에 가깝게 매겨지지 않고, reason에 시니어리티/역할 불일치가 리스크 요인으로 설명됨
- [ ] 없는 약점을 지어내서 `matches.missing`을 억지로 채우지 않음(요건을 다 충족하므로 missing이 비어있거나 거의 없는 게 정상)
- [ ] `resumeSuggestions`가 시니어 경력을 주니어 포지션에 맞게 재구성하라는 실질적 조언을 포함(과장 없이)
...
- [ ] `matches.strong` 또는 `weak`에 Python, SQL, CI/CD(GitHub Actions) 관련 항목이 포함됨(진짜 전이 가능한 증거이므로 세트 3처럼 비어있으면 안 됨)
- [ ] `matches.missing`에 Django(또는 웹 프레임워크 실무 경험)가 명시적으로 포함됨 — 테스트 코드에서 API를 "호출"해본 경험을 Django API "설계/개발" 경험으로 부풀리지 않음
- [ ] `candidateEvidence`가 QA 프로젝트의 Python/SQL 사용을 정확히 반영하되, 이를 곧 "백엔드 개발 경험"으로 재포장하지 않음
- [ ] `fitScore.total`이 세트 3(완전 불일치)보다는 높고 세트 1(강한 매치)보다는 낮은 중간대 — reason이 "부분 전이 가능하나 핵심 프레임워크 경험 부재"를 구체적으로 설명
- [ ] `miniProjects`가 QA 배경을 살리면서 Django 갭을 메우는 현실적 제안(예: 기존 pytest 경험과 연결된 Django REST API 미니 프로젝트)을 포함
- [ ] `resumeSuggestions`가 QA 자동화 경험을 백엔드 지원 관점에서 어필하도록 제안하되, 지어낸 Django 실무 경력을 주장하지 않음
...
- [ ] JD가 영어임에도 `summary`, `reason`, `evidenceSnippet`, 이력서 제안, 면접 질문 등 모든 자연어 필드가 한국어로 작성됨
- [ ] `jobRequirements.requiredSkills`의 `label`은 Node.js/TypeScript/PostgreSQL 등 고유명사를 영어 원문 그대로 유지(임의로 한글 음차 번역하지 않음)
- [ ] 영어 원문의 요건(3+ years, PostgreSQL, payment/transaction-critical)이 번역 과정에서 누락되지 않고 requiredSkills에 정확히 반영됨
- [ ] `matches.strong`에 Node.js, TypeScript, PostgreSQL, Stripe, Kafka가 실제 증거 기반으로 강한 매치로 포함됨
- [ ] "hybrid (3 days on-site)" 같은 고용형태 문구를 요건으로 잘못 끌어오지 않음
...
- [ ] `jobRequirements.requiredSkills`가 복지/문화 문단에서 항목을 만들어내지 않고, 실제 자격요건 문단의 React/TypeScript/Next.js/GraphQL만 필수로 포함
- [ ] `preferredSkills`에 Jest/RTL, 웹 접근성, 대규모 트래픽 운영이 정확히 우대 사항으로 분리됨
- [ ] 복지(맥북, 식대, 도서비 등)나 회사문화(수평적 문화, 데모데이 등) 내용이 `deliveryExpectations`/`collaboration`/`domain` 어디에도 요건처럼 들어가지 않음
- [ ] `matches.strong`에 React/TypeScript/Next.js/GraphQL이 정확히 매핑됨
- [ ] `summary`가 장문의 노이즈에 휘둘리지 않고 핵심 기술 매치를 간결하게 요약함
- [ ] `fitScore.total`이 높고, reason이 노이즈가 아닌 실제 기술 스택 대조에 기반함
...
8세트 전부 PASS면:
1. 이 파일의 "결과 기록" 8개를 캡처/요약해서 알려준다.
2. `VERIFICATION.md`에 새 게이트 로그(85%→90% 또는 결정에 따라)를 추가하고 `검증현황.md` 1번 행의 "현재 상태"를 갱신한다.
